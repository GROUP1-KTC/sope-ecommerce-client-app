...
const VoucherItem: React.FC<VoucherItemProps> = ({ title, discount, minOrder, platform, expiry, used, type }) => {
  const iconFrameColor = type === 'amount' ? 'bg-red-400' : 'bg-green-400';
  const borderColor = type === 'amount' ? 'border-l-red-500' : 'border-l-green-500';

  return (
    <div className={`bg-white text-gray-800 rounded-lg p-3 relative mb-3 shadow-md flex gap-3 border-l-4 ${borderColor}`}>
      ...
      <div className="flex-1 flex items-center gap-3">
        <div className="flex-1">
          ...
          <p className="text-[10px] mt-1.5">Hiệu lực sử dụng: {expiry}</p>
        </div>
        <button
          className={`flex-shrink-0 px-4 py-1.5 ${
            used ? 'bg-gray-500 cursor-not-allowed' : 'bg-white hover:bg-red-600 hover:text-white'
