...
const ProductList: React.FC<ProductListProps> = ({ products }) => {
	...
	const sortedProducts = [...products].sort((a, b) => {
		...
		if (sort === 'bestseller') return b.totalSold - a.totalSold;
		return 0;
	});
	...
	return (
		<div className="flex-1 pl-6">
			...
			<div className="flex items-center gap-2 mb-4 relative">
				...
				{/* Nút mới nhất */}
				<button
					className={`px-4 py-2 rounded cursor-pointer border text-sm font-medium ${sort === 'newest'
						? 'bg-red-500 text-white border-red-500'
						: 'bg-white text-gray-700 border-gray-300 hover:bg-gray-100'
						}`}
				...
				<div className="relative group">
					...
					<div className="absolute left-0 top-full mt-1 hidden group-hover:block bg-white border rounded shadow-md z-10 min-w-[150px]">
						<button
							className={`block w-full text-left px-4 py-2 text-sm ${sort === 'price_asc' ? 'bg-red-100 text-red-600' : 'hover:bg-gray-100'
								}`}
						...
						<button
							className={`block w-full text-left px-4 py-2 text-sm ${sort === 'price_desc' ? 'bg-red-100 text-red-600' : 'hover:bg-gray-100'
								}`}
				...
				{/* Phân trang */}
				<span className="ml-auto text-xs text-gray-500">{page}/8</span>
				<button
				...
				<button
					...
					disabled={page === 8}
					onClick={() => setPage((p: number) => Math.min(8, p + 1))}
				>
					<ArrowForwardIcon />
				</button>
			</div>
						...
						<div className="text-left text-sm">
							...
							<div className="flex items-center gap-2 mb-1">
								<span className="text-red-500 font-extrabold text-xl">
									₫{(product.defaultPrice ?? 0).toLocaleString('vi-VN')}
								</span>
