...
export default function TableNotPaid({ orders = [] }: { orders?: OrderGroupShop[] }) {
    const list = orders ?? [];
    ...
    return (
        <div className="overflow-x-auto border rounded text-sm">
            <table className="min-w-full">
                ...
                <tbody>
                    {list.map((o) => {
                        const confirmedEntry = o.order.statusHistory?.find((h) => h.status === 'CONFIRMED');
                        const confirmedDate = confirmedEntry?.timestamp ? dayjs(confirmedEntry.timestamp).format('DD/MM/YYYY') : '-';
                        return (
                            <tr key={o.order.orderId} className="border-t">
                                ...
                                <td className="px-4 py-2 text-orange-500">Chưa thanh toán</td>
