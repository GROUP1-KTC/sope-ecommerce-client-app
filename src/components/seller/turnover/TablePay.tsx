    ...
    return (
        <div className="overflow-x-auto border rounded text-sm">
            <table className="min-w-full">
                ...
                <tbody>
                    {list.map((o) => {
                        const deliveredEntry = o.order.statusHistory?.find(
                            (h) => h.status === 'DELIVERED',
                        );
                        const deliveredDate = deliveredEntry?.timestamp
                            ? dayjs(deliveredEntry.timestamp).format(
                                  'DD/MM/YYYY',
                              )
                            : '-';
                        return (
                            <tr key={o.order.orderId} className="border-t">
                                ...
                                <td className="px-4 py-2 text-green-600">
                                    Thanh toán đã chuyển thành công
                                </td>
                                <td className="px-4 py-2">
                                    {getPaymentLabel(
