		...
		<div className="fixed inset-0 flex items-center justify-center bg-black/40 z-50">
			<div className="bg-white rounded-lg shadow-lg w-[480px] max-h-[90vh] overflow-y-auto">
				<div className="p-6 border-b">
					<h2 className="text-lg font-semibold">Thiết lập Quảng cáo</h2>
					<p className="text-sm text-gray-500">Sản phẩm: {productId}</p>
				</div>

				<div className="p-6">
					<h3 className="font-medium mb-3">(Gói Ads 7 ngày) với giá 700000</h3>
					<div className="grid grid-cols-7 gap-2 text-sm">
						{days.map((day, idx) => {
							const isSelected = startDate && isSameDay(startDate, day);
							const active = isInRange(day);
							return (
								<button
									key={idx}
									onClick={() => handleSelect(day)}
									disabled={isBefore(day, today) || isAfter(day, maxDate)}
