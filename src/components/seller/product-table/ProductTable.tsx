...
export default function ProductTable({ products, viewMode }: Props) {
	...
	const sortedProducts = [...products].sort((a, b) => {
		const getValue = (p: ProductResponse, key: "price" | "stock" | "sold") => {
			if (key === "price") return Math.min(...p.variants.map(v => v.price));
			if (key === "stock") return p.variants.reduce((s, v) => s + (v.stock ?? 0), 0);
			if (key === "sold") return p.variants.reduce((s, v) => s + (v.sold ?? 0), 0);
			return 0;
		};
							...
							<div className="p-3 text-sm flex flex-col justify-between h-31">
								...
								<div>
									...
									<div className="text-xs text-gray-500 mt-1">
										Kho hàng: {product.variants.reduce((sum, v) => sum + (v.stock ?? 0), 0)}
									</div>
									<div className="text-xs text-gray-500">
										Đã bán: {product.variants.reduce((sum, v) => sum + (v.sold ?? 0), 0)}
									</div>
								</div>
							</div>
						</Link>
				...
				<table className="min-w-full bg-white">
					<thead>
						<tr className="bg-gray-50 text-gray-700 text-sm">
							...
							<th className="p-3  text-center">Chất lượng nội dung</th>
					...
					<tbody>
						{products.map((product) => {
							...
							const firstVariant = hasOnlyOneVariant ? variants[0] : null;
							const isSimpleProduct = hasOnlyOneVariant && (!firstVariant?.attributes || firstVariant.attributes.length === 0);
							...
							return (
								<React.Fragment key={product.productId}>
									{/* Row cho product */}
									<tr key={product.productId} className="hover:bg-orange-50 mb-4">
										...
										<td className="p-3 align-top text-center">
											{isSimpleProduct
												? firstVariant?.sold ?? 0
												: variants.reduce((t, v) => t + (v.sold ?? 0), 0)}
										</td>
												...
												(() => {
													...
													return (
														<>
															...
															{minPrice !== maxPrice && <> - ₫{maxPrice.toLocaleString("vi-VN")}</>}
														</>
													);
												})()
											)}
											<button onClick={() => openEditModal(product, "price")} className="cursor-pointer ml-1 text-yellow-500 hover:text-orange-500">
												<Pencil size={14} />
											</button>
										</td>
										...
										<td className="p-3 align-top text-center">
											{isSimpleProduct
												...
												: variants.reduce((t, v) => t + (v.stock ?? 0), 0)
											}
											<button onClick={() => openEditModal(product, "stock")} className="cursor-pointer ml-1 text-yellow-500 hover:text-orange-500">
												<Pencil size={14} />
											</button>
										</td>
										{/* Chất lượng nội dung */}
										<td className="p-3 align-top text-center">Đạt chuẩn</td>
									...
									{!isSimpleProduct && (
										<AnimatePresence initial={false}>
											{(showAll ? variants : variants.slice(0, 2)).map((variant, idx) => (
												<motion.tr
													key={variant.productVariantId || idx}
												...
												>
													...
													<td className="p-3 align-top flex gap-3 pl-8 ">
														...
														<div>
															...
															<div className="text-xs text-gray-500">
																Model ID: {variant.productVariantId}
															</div>
														</div>
													</td>
													<td className="p-3 align-top text-center">{variant.sold ?? 0}</td>
													<td className="p-3 align-top text-center">
									...
									{!isSimpleProduct && variants.length > 2 && (
										<tr>
												...
												<div className="flex justify-center">
													...
													>
														{showAll ? (
															...
															<>
																Xem thêm (còn {variants.length - 2} phân loại) <ChevronDown size={14} />
															</>
														)}
													</button>
												</div>
