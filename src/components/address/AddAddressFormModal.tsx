...
const AddressFormModal = ({ isOpen, onClose, onSubmit }: AddressFormModalProps) => {
  ...
  useEffect(() => {
    if (addressData.province) {
      fetch(`https://provinces.open-api.vn/api/p/${addressData.province}?depth=2`)
        .then((res) => res.json())
  ...
  useEffect(() => {
    if (addressData.district) {
      fetch(`https://provinces.open-api.vn/api/d/${addressData.district}?depth=2`)
        .then((res) => res.json())
  ...
  const handleSubmit = (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    const provinceName = provinces.find((p) => p.code === parseInt(addressData.province))?.name || '';
    const districtName = districts.find((d) => d.code === parseInt(addressData.district))?.name || '';
    const wardName = wards.find((w) => w.code === parseInt(addressData.ward))?.name || '';
    ...
    onSubmit(newAddress);
    setAddressData({ name: '', phone: '', province: '', district: '', ward: '', detailedAddress: '', isDefault: false });
  };
      ...
      <div className="relative bg-white rounded-lg p-10 w-full max-w-2xl shadow-lg">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="flex items-center">
            <label className="block text-gray-700 text-sm font-bold w-1/4">Họ tên</label>
            <input
          ...
          <div className="flex items-center">
            <label className="block text-gray-700 text-sm font-bold w-1/4">Số điện thoại</label>
            <input
              className="w-3/4 p-2 border rounded focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent"
              type="tel"
              name="phone"
          ...
          <div className="flex items-center">
            <label className="block text-gray-700 text-sm font-bold w-1/4">Tỉnh/Thành phố</label>
            <select
          ...
          <div className="flex items-center">
            <label className="block text-gray-700 text-sm font-bold w-1/4">Quận/Huyện</label>
            <select
              className="w-3/4 p-2 border rounded focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent cursor-pointer transition"
              name="district"
          ...
          <div className="flex items-center">
            <label className="block text-gray-700 text-sm font-bold w-1/4">Phường/Xã</label>
            <select
              className="w-3/4 p-2 border rounded focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent cursor-pointer transition"
              name="ward"
          ...
          <div className="flex items-center">
            <label className="block text-gray-700 text-sm font-bold w-1/4">Địa chỉ chi tiết</label>
            <input
              className="w-3/4 p-2 border rounded focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent"
              type="text"
          ...
          <div className="flex items-center">
            <label className="block text-gray-700 text-sm font-bold w-1/4">Mặc định</label>
            <div className="w-3/4 flex items-center">
              ...
              <span className="ml-3 text-sm text-gray-600">Đặt làm mặc định</span>
            </div>
          </div>
