  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col items-center py-8">
      <div className="w-full max-w-5xl">
        <div className="text-sm text-gray-500 mb-4 hover:bg-red-200">Trang chủ &gt; Tất cả Danh mục</div>
        {/* Alphabet navigation */}
        <div className="flex justify-center gap-2 mb-6 select-none">
          {alphabet.map((ch) =>
            groups[ch] ? (
              ...
              <span key={ch} className="text-gray-300 px-2 cursor-not-allowed">{ch}</span>
            )
          )}
        </div>
          ...
          (ch) =>
            groups[ch] && (
              <div key={ch} id={`cat-${ch}`} className="mb-12">
                <div className="text-3xl font-bold text-gray-700 mb-4">{ch}</div>
