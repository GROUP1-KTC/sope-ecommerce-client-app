...
export default function OrderDetail() {
	...
	if (isError || !orderDetail || !order) return <div>Không tìm thấy đơn hàng</div>;
	...
	const calculateRevenue = (order: any) => {
		...
		const commission =
			order.items?.reduce((sum: number, item: any) => {
				return sum + item.price * item.quantity * (item.commissionFeePercent / 100);
			}, 0) ?? 0;
				...
				<div className="md:col-span-2 space-y-6">
					...
					<div className="bg-white rounded shadow p-4">
						...
						<div className="text-sm text-gray-700 space-y-1">
							...
							<div>
								...
								{shippingProviders[order.shippingRateId] ?? "Không rõ"}
							</div>
							<div>
					...
					<div className="bg-white rounded shadow p-4">
						...
						<div className="text-sm text-gray-700 space-y-1">
							...
							<div>
								Địa chỉ giao hàng: {`${orderDetail.shippingAddress.street}, ${orderDetail.shippingAddress.ward}, ${orderDetail.shippingAddress.district}, ${orderDetail.shippingAddress.city}, ${orderDetail.shippingAddress.country}`}
							</div>
						</div>
					</div>
					...
					<div className="bg-white rounded shadow p-4">
						...
						<table className="w-full  text-sm">
							...
							<tbody>
								{items.map((item, idx) => (
									<tr key={item.productVariantId ?? idx} className="align-top items-center">
										<td className="px-3 py-2 items-center text-center">{idx + 1}</td>
							...
							<div className="flex justify-between">
								...
								<span>
									...
									{(
										order.items?.reduce(
											(sum: number, item: any) =>
												sum + item.price * item.quantity * (item.commissionFeePercent / 100),
											0
										) ?? 0
					...
					<div className="mt-6 space-y-6">
						...
						<div className="bg-white rounded shadow p-4">
							...
							<div className="space-y-2 text-sm">
								...
								<div className="flex justify-between">
									...
									<span>{order.shippingCharges?.toLocaleString("vi-VN")} đ</span>
								</div>

								{/* Hiển thị danh sách mã giảm giá / voucher */}
								{order.orderDiscounts?.map((d: any, idx: number) => (
						...
						<div className="bg-white rounded shadow p-4 flex items-center justify-between">
							...
							<span className="text-3xl font-semibold text-blue-800">
								{calculateRevenue(order).toLocaleString("vi-VN")} đ
							</span>
						</div>
						...
						<ul className="text-sm text-gray-700 space-y-2">
							{order.statusHistory?.map((s, idx) => (
								<li key={idx} className="flex justify-between">
									<span className={statusColors[s.status as keyof typeof statusColors]}>
										{s.status}
									</span>
									<span>{new Date(s.timestamp).toLocaleString("vi-VN")}</span>
								</li>
							))}
						</ul>
					</div>
