  ...
  return (
    <div className="w-full flex justify-center bg-gray-50 py-8">
      <div className="bg-white rounded-xl shadow p-6 max-w-6xl w-full">
        <h2 className="text-lg font-semibold mb-4 text-gray-800">TOP TREND CLOTHING FOR MEN</h2>
        <div className="grid grid-cols-5 gap-4">
          {trendingStyles.map((item, idx) => (
            <div key={idx} className="border rounded-lg bg-white flex flex-col items-center p-2">
              ...
              <div className="w-full text-center">
                <div className="font-semibold text-sm mb-1 uppercase">{item.name}</div>
                
                <div >
                    <span>Từ</span>
                    <span className="text-red-500 text-base font-bold">  {item.price.toLocaleString()}₫</span>
                    </div>
              </div>
            </div>
          ))}
