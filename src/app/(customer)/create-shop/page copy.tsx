...
export default function CreateShopMultiStep() {
  ...
  const validateStep = (s: number) => {
    ...
    if (s === 1) {
      ...
      if (!phone.trim() || phone.trim().length < 7) e.phone = "SĐT không hợp lệ";
      if (addresses.length === 0) e.addresses = "Phải có ít nhất 1 địa chỉ lấy hàng";
    } else if (s === 2) {
      ...
      if (!idFront || !selfie) e.idDocs = "Cần ít nhất ảnh mặt trước của giấy tờ và selfie";
    }
    setErrors(e);
  ...
  const handleFinalSubmit = () => {
    ...
    if (!ok) {
      alert("Có lỗi trong form. Kiểm tra lại các bước.");
      return;
    }
    ...
    alert("Đăng ký hoàn tất (mock). Kiểm tra console để xem payload.");
    // reset or navigate
  };
        ...
        <AddressModal
          ...
          onAdd={(addr) => {
            setAddresses((prev) => {
              ...
              const isDefault = prev.length === 0 ? true : addr.isDefault ?? false;
              const next = [...prev, { ...addr, id: cryptoRandomId(), isDefault }];
              // ensure single default
              if (isDefault) {
                return next.map((a) => ({ ...a, isDefault: a.id === addr.id }));
              }
              return next;
            });
...
function Step1({
  ...
  return (
    <div>
      <h3 className="text-lg font-semibold mb-4">Step 1 — Thông tin cơ bản & địa chỉ lấy hàng</h3>
      ...
      <div className="grid grid-cols-1 gap-4">
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            <label className="block text-sm font-medium">Địa chỉ lấy hàng</label>
            <button
              type="button"
          ...
          {addresses.length === 0 ? (
            <div className="text-sm text-gray-500">Chưa có địa chỉ nào. Thêm để tiếp tục.</div>
          ) : (
            <ul className="space-y-2">
              {addresses.map((a) => (
                <li key={a.id} className="flex items-center justify-between border rounded p-2">
                  <div>
                    <div className="font-medium">{a.name} {a.isDefault && <span className="text-xs bg-green-100 px-2 ml-2 rounded">Mặc định</span>}</div>
                    <div className="text-sm text-gray-600">{a.phone} • {a.street}, {a.ward}, {a.district}, {a.city}</div>
                  </div>
                  <div className="flex items-center gap-2">
                    {!a.isDefault && (
                      <button onClick={() => toggleDefaultAddress(a.id)} className="text-sm text-blue-600">Đặt mặc định</button>
                    )}
                    <button onClick={() => removeAddress(a.id)} className="text-sm text-red-500">Xóa</button>
                  </div>
                </li>
              ))}
            </ul>
          )}
          {errors.addresses && <p className="text-red-500 text-sm mt-1">{errors.addresses}</p>}
        </div>
      </div>
    </div>
  );
...
function Step2({
  ...
  return (
    <div>
      <h3 className="text-lg font-semibold mb-4">Step 2 — Thông tin về shop</h3>
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          {logoPreview ? (
            ...
            <img src={logoPreview} alt="logo" className="w-28 h-28 object-cover rounded mt-3 border" />
          ) : (
            <div className="text-sm text-gray-500 mt-2">Chưa có ảnh</div>
          )}
        </div>
      </div>
    </div>
...
function Step3({
  ...
  return (
    <div>
      <h3 className="text-lg font-semibold mb-4">Step 3 — Thông tin thuế / doanh nghiệp</h3>

      <div className="space-y-4">
        <div className="flex items-center gap-3">
          <input id="isMall" type="checkbox" checked={isMall} onChange={(e) => setIsMall(e.target.checked)} />
          <label htmlFor="isMall" className="text-sm">Đăng ký là Mall (cần mã số thuế)</label>
        </div>
        ...
        {isMall && (
          <>
            <div>
              ...
              <input value={taxCode} onChange={(e) => setTaxCode(e.target.value)} className="w-full border rounded p-2" />
              {errors.taxCode && <p className="text-red-500 text-sm mt-1">{errors.taxCode}</p>}
            </div>

            <div>
              <label className="block text-sm font-medium mb-1">Giấy tờ thuế (upload, nếu có)</label>
              <input type="file" accept="application/pdf,image/*" onChange={(e) => setTaxFile(e.target.files?.[0] ?? null)} />
              {taxFilePreview ? (
                // eslint-disable-next-line @next/next/no-img-element
                <img src={taxFilePreview} alt="tax" className="w-40 h-32 object-cover mt-2 border rounded" />
              ) : (
                <div className="text-sm text-gray-500 mt-2">Chưa có file</div>
              )}
            </div>
          </>
        )}
...
function Step4({
  ...
  return (
    <div>
      <h3 className="text-lg font-semibold mb-4">Step 4 — Xác minh định danh</h3>

      <div className="space-y-4">
        <div>
          <label className="text-sm block mb-1">Loại giấy tờ</label>
          <select value={idType} onChange={(e) => setIdType(e.target.value as any)} className="border rounded p-2">
            <option value="CITIZEN">Chứng minh nhân dân / Căn cước</option>
            <option value="PASSPORT">Hộ chiếu</option>
          </select>
        </div>

        <div>
          <label className="text-sm block mb-1">Họ tên (trên giấy tờ)</label>
          <input value={idName} onChange={(e) => setIdName(e.target.value)} className="w-full border rounded p-2" />
          {errors.idName && <p className="text-red-500 text-sm mt-1">{errors.idName}</p>}
        </div>

        <div>
          <label className="text-sm block mb-1">Số giấy tờ</label>
          <input value={idNumber} onChange={(e) => setIdNumber(e.target.value)} className="w-full border rounded p-2" />
          {errors.idNumber && <p className="text-red-500 text-sm mt-1">{errors.idNumber}</p>}
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div>
            <label className="text-sm block mb-1">Ảnh giấy tờ (mặt trước)</label>
            <input type="file" accept="image/*" onChange={(e) => setIdFront(e.target.files?.[0] ?? null)} />
            {previewIdFront && (
              // eslint-disable-next-line @next/next/no-img-element
              <img src={previewIdFront} alt="front" className="w-full h-32 object-cover mt-2 border rounded" />
            )}
          </div>
          <div>
            <label className="text-sm block mb-1">Ảnh giấy tờ (mặt sau)</label>
            <input type="file" accept="image/*" onChange={(e) => setIdBack(e.target.files?.[0] ?? null)} />
            {previewIdBack && (
              // eslint-disable-next-line @next/next/no-img-element
              <img src={previewIdBack} alt="back" className="w-full h-32 object-cover mt-2 border rounded" />
            )}
          </div>
          <div>
            <label className="text-sm block mb-1">Selfie (cầm giấy tờ)</label>
            <input type="file" accept="image/*" onChange={(e) => setSelfie(e.target.files?.[0] ?? null)} />
            {previewSelfie && (
              // eslint-disable-next-line @next/next/no-img-element
              <img src={previewSelfie} alt="selfie" className="w-full h-32 object-cover mt-2 border rounded" />
            )}
          </div>
        </div>

        {errors.idDocs && <p className="text-red-500 text-sm mt-1">{errors.idDocs}</p>}
      </div>
    </div>
  );
}
...
function Step5({ summary }: { summary: any }) {
  return (
    <div>
      <h3 className="text-lg font-semibold mb-4">Step 5 — Xác nhận & Hoàn tất</h3>
      ...
      <div className="space-y-3">
        ...
        <div>
          ...
          <div className="text-gray-700">
            {summary.addresses.length === 0 ? "Chưa có" : (
              <ul className="list-disc ml-6">
                {summary.addresses.map((a: Address) => (
                  <li key={a.id}>{a.name} — {a.phone} — {a.street}, {a.ward}, {a.district}, {a.city} {a.isDefault && "(Mặc định)"}</li>
                ))}
              </ul>
            )}
          </div>
        ...
        <div>
          ...
          <div className="text-gray-700">{summary.isMall ? `Mall — MST: ${summary.taxCode || "-"}` : "Không"}</div>
        </div>
        ...
        <div className="mt-4">
          <label className="flex items-center gap-2">
            ...
            <span className="text-sm text-gray-700">Tôi xác nhận thông tin trên là đúng</span>
          </label>
        </div>
      </div>
    </div>
      ...
      <div className="relative bg-white rounded-lg shadow-xl w-full max-w-2xl p-6">
        ...
        <form onSubmit={submit} className="space-y-3">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className="text-sm block mb-1">Người nhận</label>
              <input name="name" value={form.name} onChange={handle} className="w-full border rounded p-2" required />
            </div>
            <div>
              <label className="text-sm block mb-1">Số điện thoại</label>
              <input name="phone" value={form.phone} onChange={handle} className="w-full border rounded p-2" required />
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            <input id="isDefault" name="isDefault" type="checkbox" checked={form.isDefault} onChange={handle} />
            <label htmlFor="isDefault" className="text-sm">Đặt là địa chỉ mặc định</label>
          </div>
