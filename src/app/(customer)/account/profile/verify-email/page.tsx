...
const VerifyEmailPage = () => {
  ...
  const [errors, setErrors] = useState<{ verificationCode: string }>({ verificationCode: '' });

  const changeEventHandler = (e: React.ChangeEvent<HTMLInputElement>) => {
    setInput({ ...input, [e.target.name]: e.target.value });
    if (e.target.value.length !== 6) {
      setErrors({ verificationCode: 'OTP must be 6 digits' });
    } else {
        ...
        <div className="flex flex-1">
          <div className="flex-1 p-6 bg-white rounded-lg shadow-md my-6">
            ...
            <div className="text-gray-600 mb-4">
              OTP đã được gửi đến địa chỉ email của bạn. Vui lòng nhập mã bên dưới.
            </div>
            <hr className="my-4 border-gray-300" />

            <form onSubmit={handleCodeSubmit} className="px-4 md:px-8 space-y-6">
              <div>
